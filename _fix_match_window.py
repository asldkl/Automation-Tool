...
old_btn = """text="\U0001f50d ORB图片匹配上传\""""
new_btn = 'text="\U0001f50d 图片匹配上传"'
assert old_btn in content, "Button text not found"
...
# === 3. Rename _orb_batch_upload title docstring ===
content = content.replace(
    '"""ORB批量匹配：用户上传一张截图，自动与所有默认模板匹配，找到最佳对应模板"""',
    '"""图片匹配上传：直接打开窗口，在窗口中上传图片进行特征匹配"""'
)

# === 4. Replace _orb_batch_upload method ===
old_batch = '''    def _orb_batch_upload(self):
        """图片匹配上传：直接打开窗口，在窗口中上传图片进行特征匹配"""
        import cv2
        import numpy as np
        ...
        if user_img is None:
            messagebox.showerror("错误", "无法读取图片文件，请确认格式正确。路径可能包含中文或特殊字符。", parent=self.win)
            return
        ...
        # 对每个默认模板执行 ORB 匹配
        results = []  # [(score, match_count, ratio, var_name, name, rel_path), ...]
        ...
        # 显示匹配结果窗口，让用户选择
        self._show_orb_results(results, file_path)

    def _show_orb_results(self, results, file_path):
...
new_batch = '''    def _image_match_upload(self):
        """图片匹配上传：直接打开窗口，在窗口中上传图片进行匹配"""
        self._show_match_results()

    def _run_matching(self, img_data, algorithm="ORB"):
        """对上传的图片执行特征匹配，返回排序后的结果列表"""
        import cv2
        import numpy as np
        ...
        if algorithm == "ORB":
            ...
            for var_name, rel_path, name, hint in self.capture_list:
                ...
                if template_img is None:
                    continue
                # pHash: 缩放到8x8 -> DCT -> 取左上8x8 -> 比较汉明距离
                def _phash(img):
                    img8 = cv2.resize(img, (32, 32), interpolation=cv2.INTER_AREA)
                    img32f = np.float32(img8)
                    dct = cv2.dct(img32f)
                    dct_low = dct[:8, :8]
                    avg = np.mean(dct_low[1:])  # 跳过DC分量
                ...
                score = max(0, 64 - hamming * 2)  # 64为最大可能，分越高越相似
    ...
    def _show_match_results(self, file_path=None):
        ...
        current_results = [[]]  # [(score, match_count, ratio, var_name, name, rel_path)]
        current_algo = ["ORB"]
        ...
        algo_label = ttk.Label(win, textvariable=algo_var,
                               font=("Microsoft YaHei UI", 9, "bold"),
                               foreground="#555")
        algo_label.pack(pady=(10, 0), padx=15, anchor="w")

        conf_label = ttk.Label(win, textvariable=conf_var,
                               font=("Microsoft YaHei UI", 12, "bold"),
                               foreground="#e67e22")
        conf_label.pack(pady=(0, 2), padx=15, anchor="w")
        ...
        def load_and_display(cv, img_path, max_size=(280, 280)):
            try:
                ...
                tk_img = ImageTk.PhotoImage(pil_img)
                cv.delete("all")
                cv.create_image(pil_img.width // 2 + 10, pil_img.height // 2 + 10,
        ...
        upload_btn = ttk.Button(nav_frame, text="📁 上传图片", style="Accent.TButton",
        ...
        ttk.Label(algo_frame, textvariable=algo_var_label,
                  font=("Microsoft YaHei UI", 9, "bold")).pack(side=tk.LEFT, padx=(0, 10))

        def switch_algo():
            if current_algo[0] == "ORB":
                current_algo[0] = "pHash"
                algo_var_label.set("当前算法：pHash（感知哈希）")
        ...
        algo_btn = ttk.Button(algo_frame, text="切换到 pHash",
                              command=switch_algo, width=14)
...
assert old_batch in content, "Old batch/show methods not found!"
content = content.replace(old_batch, new_batch)

# === 5. Fix the button command reference ===
content = content.replace(
    "command=self._orb_batch_upload",
    "command=self._image_match_upload"
)
