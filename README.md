...
- **系统托盘** — 静默运行，后台执行任务
- **电源管理** — 唤醒电脑、自动关机、定时开机
- **邮件通知** — 执行完成后自动发送运行报告到邮箱，包含账号统计、已选操作、运行模式、一键出售结果、冷却状态、每个账号资产数；手动终止时不发送
- **账号冷却管理** — 防止账号频繁执行，支持自定义冷却时间（默认8小时），含冷却状态查看、重置和暂停/恢复
...
- **运行完成后延迟关机** — 所有账号处理完毕后可设置0-5分钟延迟自动关机（手动终止时不执行）
